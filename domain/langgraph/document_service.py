...
class DocumentService:
    ...
    async def process_code_change(self, code_change_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if existing_doc:
                ...
                logger.info(f"Adding commit {code_change.commit_sha} to existing repository document: {repository_name}")
    ...
    async def _append_to_repository_document(self, session: Session, document: Document, code_change: CodeChange) -> Dict[str, Any]:
        """기존 저장소 문서에 새 커밋 내용 추가"""
        try:
            # 파일 변경사항 가져오기
            file_changes = session.query(FileChange).filter(
    ...
    async def _update_existing_document(self, session: Session, document: Document, code_change: CodeChange) -> Dict[str, Any]:
        """기존 문서 업데이트"""
        try:
            # 파일 변경사항 가져오기
            file_changes = session.query(FileChange).filter(
    ...
    def _prepare_cumulative_code_input(self, document: Document, code_change: CodeChange, file_changes: List[FileChange]) -> CodeChangeInput:
    ...
    def get_documents(self, repository_name: Optional[str] = None, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        """문서 목록 조회"""
        session = SessionLocal()
        try:
